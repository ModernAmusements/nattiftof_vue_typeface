...
export default {
  ...
  mounted() {
   window.addEventListener("load", function() {
    ...
    let n = ()=>{
        1 == t && (function(t) {
            1 == t ? (document.documentElement.setAttribute("data-theme", "dark"),
            sessionStorage.setItem("theme", "dark"),
            (e = "#FFFFFF")) : (document.documentElement.setAttribute("data-theme", "light"),
      ...
      , h = ()=>{
        ...
        for (let e = 0; e < t.length; e++) {
            let i = t[e].offsetWidth
              , n = t[e].offsetHeight
              , o = Math.floor(i * n);
            t[e].innerHTML = "<span class='sl-z'>" + l(o) + "<span class='f-sup'> px2</span></span>";
        }
    }
    ;
    h(),
    document.getElementById("navn").addEventListener("mouseenter", ()=>{
        window.outerWidth > 1024 && !d && document.getElementById("navh").classList.add("nav-hovered");
    }
    ),
    document.getElementById("navn").addEventListener("mouseleave", ()=>{
        window.outerWidth > 1024 && !d && document.getElementById("navh").classList.remove("nav-hovered");
    }
    );
    ...
    window.outerWidth < 1025 || d ? ((v.ballR *= 2.5),
    (v.typeS *= 2.5),
    (m.innerHTML = "Tap to start the game")) : (m.innerHTML = "Click to start the game");
    ...
    f.on(w, "beforeUpdate", function(t) {
        ...
        W && (T > 0.8 * y && I.position.y < 0.8 * y && (B(++E),
        ...
        k.moveShape(),
        W ? I.position.y > 1.2 * k.y ? L(1) : new Date().getTime() > S && L(0) : p.setPosition(I, {
            x: D.x,
            y: 0.88 * k.y
        });
    });
    ...
    function L(t) {
        ...
        (m.innerHTML = t ? "You've lost it! " + e + " to get a new ball." : "You're out of time! " + e + " to start a new game.");
    }
    let k = new (class {
        constructor(t, e, i) {
            (this.s = t),
        ...
        createShape() {
            (this.base = Matter.Bodies.rectangle(this.x, this.y, 14 * this.s, 2 * this.s, {
                isStatic: !0,
                render: {
                    fillStyle: e,
                    strokeStyle: e,
            ...
            (this.left = Matter.Bodies.rectangle(this.x - 7 * this.s, this.y - this.s, 2 * this.s, 4 * this.s, {
                isStatic: !0,
                render: {
                    fillStyle: e,
                    strokeStyle: e,
            ...
            (this.right = Matter.Bodies.rectangle(this.x + 7 * this.s, this.y - this.s, 2 * this.s, 4 * this.s, {
                isStatic: !0,
                render: {
                    fillStyle: e,
                    strokeStyle: e,
    ...
    function R(t) {
        g > 1024 ? ((H.x = t.layerX),
        (H.y = t.layerY)) : (H.x = document.getElementById("volume").value);
    }
    let D = {
        x: 0.5 * g,
        y: 0
    ...
    (window.outerWidth < 1025 || d) && ((document.getElementById("volume").max = g),
